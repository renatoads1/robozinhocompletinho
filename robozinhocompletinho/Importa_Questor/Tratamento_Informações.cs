...
namespace Importa_Questor
{
    class Tratamento_Informações
    {
        ...
        public void UpdateFila(string id, string status)
        {
            
            //metodo que altera o status do banco de dados de acordo com o caminho passado
            ...
            {
                string StrSql = "update " + bdMysql + $@".PBHXmlPrestListFile set status ='" + status + "' where id =  '" + id + "';";
                var comando = new MySqlCommand(StrSql, conex);
                conex.Open();
                comando.ExecuteNonQuery();
                conex.Close();
